...
{
    echo
    echo "upload bootloder (for easyHOME board) command syntax:"
    echo
...
../../../AVR-GCC/bin/avrdude -p atmega2560 -P $upload_device -c stk500v1 -b 19200 -C ../../../AVR-GCC/etc/avrdude.conf -U flash:w:stk500boot_v2_easyHOME.hex:i -Ulock:w:0x0F:m
if [[ $? != 0 ]]
then
    upload_ko
fi
